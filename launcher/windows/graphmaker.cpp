...
static char* defaultDirs[] = { "graphmaker1",
                               "extra",
                               NULL };
...
int WINAPI
    ...
    sprintf(buf, "%s\\etc\\graphmaker.conf", topdir);
    parseConfigFile(buf);
    ...
    sprintf(buf, "%s\\etc\\graphmaker.conf", userdir);
    parseConfigFile(buf);
    ...
    sprintf(cmdline2, "\"%s\" %s --branding graphmaker --clusters \"%s\" --userdir \"%s\" %s %s",
            nbexec,
            jdkswitch,
            dirs,
            userdir,
